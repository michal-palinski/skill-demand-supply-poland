...
Generate data for Datawrapper chart.
"""
...
category_stats.columns = ['category', 'job_offers', 'unique_titles']
...
# Add example occupation
examples = []
for cat in category_stats['category']:
    example = kzis_df[kzis_df['category'] == cat]['nazwa'].iloc[0] if len(kzis_df[kzis_df['category'] == cat]) > 0 else ""
...
print(f"Total unique job titles: {matches_df['job_count'].count():,}")
print()
print(category_stats.to_string(index=False))
print("=" * 100)

# Save for Datawrapper
datawrapper_data = category_stats[['category', 'category_name', 'job_offers', 'percentage', 'example']].copy()
